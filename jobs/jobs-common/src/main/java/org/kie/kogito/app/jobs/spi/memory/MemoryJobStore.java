...
public class MemoryJobStore implements JobStore {
    ...
    @Override
    public synchronized List<JobDetails> loadActiveJobs(JobContext jobContext, Date maxWindowsLoad) {
        List<JobDetails> copyJobDetails = new ArrayList<>();
        for (JobDetails jobDetails : jobs.values().stream().filter(j -> new Date(j.getExecutionTimeout()).before(maxWindowsLoad)).toList()) {
