...
public class VertxJobScheduler implements JobScheduler, Handler<Long> {
    ...
    private void syncWithJobStores() {
        ...
        for (JobDetails currentJobDetails : jobDetailsList) {
            jobsScheduled.compute(currentJobDetails.getId(), (jobId, timerInfo) -> {
                ...
                if (timerInfo.timeout().after(maxWindowsLoad)) {
                ...
                if (timerInfo.timeout().before(maxWindowsLoad) && !timerInfo.timeout().equals(new Date(currentJobDetails.getExecutionTimeout()))) {
    ...
    public String reschedule(JobDescription jobDescription) {
        JobDetails jobDetails = JobDetailsHelper.newScheduledJobDetails(jobDescription);
        doCancel(jobDetails);
        jobStore.update(jobContextFactory.newContext(), jobDetails);
        JobDetails rescheduledJobDetails = doSchedule(jobDetails);
    ...
    private TimerInfo addTimerInfo(JobDetails jobDetails) {
        ...
        Long delay = Math.max(1, jobDetails.getExecutionTimeout() - System.currentTimeMillis());
        ...
        return new TimerInfo(jobDetails.getId(), timerId, new Date(jobDetails.getExecutionTimeout()));
    }
    ...
    private JobDetails doCancel(JobDetails jobDetails) {
        this.jobsScheduled.compute(jobDetails.getId(), (jobId, timerInfo) -> {
            removeTimerInfo(timerInfo);
            return null;
        });
    ...
    private JobDetails doExecute(JobDetails jobDetails) {
        ...
        this.jobsScheduled.remove(jobDetails.getId());
