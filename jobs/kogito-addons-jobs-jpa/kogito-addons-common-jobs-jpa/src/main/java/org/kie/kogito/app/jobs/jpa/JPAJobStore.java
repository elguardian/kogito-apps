...
public class JPAJobStore implements JobStore {
    ...
    @Override
    public List<JobDetails> loadActiveJobs(JobContext jobContext, Date maxWindowsLoad) {
        EntityManager entityManager = jobContext.getContext();

        return entityManager.createQuery("SELECT o FROM JobDetailsEntity o WHERE o.status IN (:activeStatus) AND o.executionTimeout <= :maxWindowsLoad", JobDetailsEntity.class)
                .setParameter("activeStatus", JOB_ACTIVE_STATUSES)
                .setParameter("maxWindowsLoad", maxWindowsLoad.getTime())
                .getResultList()
                .stream()
